...
$el("style", {
	textContent: `
    .selfNode_style-model-info {
        color: white;
        font-family: sans-serif;
        max-width: 90vw;
    }
    .selfNode_style-model-content {
        display: flex;
        flex-direction: column;
        overflow: hidden;
    }
    .selfNode_style-model-info h2 {
        text-align: center;
        margin: 0 0 10px 0;
    }
    .selfNode_style-model-info p {
        margin: 5px 0;
    }
    .selfNode_style-model-info a {
        color: dodgerblue;
    }
    .selfNode_style-model-info a:hover {
        text-decoration: underline;
    }
    .selfNode_style-model-tags-list {
        display: flex;
    ...
    .selfNode_style-model-tags-sel-list {
        display: flex;
        align-content: flex-start;
        flex-wrap: wrap;
        list-style: none;
    ...
    .selfNode_style-model-tag--selected span::before {
        content: "✅";
        position: absolute;
    ...
    .selfNode_style-model-tag:hover {
        outline: 2px solid dodgerblue;
    }
    .selfNode_style-model-tag p {
        margin: 0;
    }
    .selfNode_style-model-tag span {
        text-align: center;
        border-radius: 5px;
        background-color: dodgerblue;
        color: #fff;
    ...
    .selfNode_style-model-metadata .comfy-modal-content {
        max-width: 100%;
    }
    .selfNode_style-model-metadata label {
        margin-right: 1ch;
        color: #ccc;
    }
    
    .selfNode_style-model-metadata span {
        color: dodgerblue;
    }
    
    .selfNode_style-preview {
        // max-width: 50%;
        // margin-left: 10px;
        position: relative;
    }
    .selfNode_style-preview img {
        max-height: 300px;
    }
    .selfNode_style-preview button {
        position: absolute;
        font-size: 12px;
        bottom: 10px;
        right: 10px;
    ...
    .selfNode_style-model-notes {
        background-color: rgba(0, 0, 0, 0.25);
        padding: 5px;
        margin-top: 5px;
    }
    .selfNode_style-model-notes:empty {
        display: none;
    }    
...
async function getStyles(name) {
    if(pb_cache[name])
        ...
        const resp = await api.fetchApi(`/selfNode/getStyles?name=${name}`);
        if (resp.status === 200) {
...
function displayImg(imgName) {  
    ...
    if(imgName) {
        img.src = `/selfNode/getImage?type=styles&&name=${imgName}.jpg`;
    }
    ...
    Object.keys(tags).forEach((k) => {
        rlist.push($el(
            "li.selfNode_style-model-tag",
            {
                ...
                $: (el) => {
                    el.onclick = () => {
                        el.classList.add("selfNode_style-model-tag--del");
                    };
                    el.onmousemove = () => {
    ...
    tags.forEach((k,i) => {
        ...
        rlist.push($el(
            "li.selfNode_style-model-tag",
            {
                ...
                $: (el) => {
                    el.onclick = () => {
                        el.classList.toggle("selfNode_style-model-tag--selected");
                    };
                    el.onmousemove = () => {
                        displayImg(el.dataset.imgName)
                    };
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        if (names.indexOf(nodeData.name)>=0) {
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                //stylesEl.inputEl.classList.add("selfNode-model-notes");
                const list = $el("ol.selfNode_style-model-tags-list",[]);
                const lists = $el("ol.selfNode_style-model-tags-sel-list",[]);
                let styles=this.addDOMWidget('styles',"list",$el('div.selfNode_style-preview',[$el('button',{
                    textContent:'清除全部选择',
                    style:{},
                    onclick:()=>{
                            styles.element.children[1].querySelectorAll(".selfNode_style-model-tag--selected").forEach(el => {
                                el.classList.remove("selfNode_style-model-tag--selected");
                            })
                    ...
                    src:'/selfNode/getStyleImage?name=abstract_expressionism'})]));
                let st_values=''
                    ...
                    set: (x) => {
                        ...
                        if(st_values){
                            ...
                            if(pb_cache[st_values]){
                                    ...
                                    styles.element.children[1].querySelectorAll(".selfNode_style-model-tag").forEach(el => {
                                    if(this.properties["values"].includes(el.dataset.tag)){
                                        el.classList.add("selfNode_style-model-tag--selected");
                                    }
                ...
                Object.defineProperty(styles, "value", {
                    ...
                    get: () => {
                        ...
                        let delList=[]
                        styles.element.children[3].querySelectorAll(".selfNode_style-model-tag--del").forEach(el => {
                            delList.push(el.dataset.tag)
                        })
                        styles.element.children[1].querySelectorAll(".selfNode_style-model-tag").forEach(el => {
                            if(el.classList.value.indexOf("selfNode_style-model-tag--selected")>=0&&!delList.includes(el.dataset.tag)){
                                if(!this.properties["values"].includes(el.dataset.tag)){
