...
async function getGroupNode(missingNodeTypes) {
    const resp = await api.fetchApi(`/selfNode/groupNode`, { cache: "no-store" });
    if (resp.status === 200) {
...
async function saveGroupNode(nodeName, nodeData) {
	try {
		const response = await api.fetchApi("/selfNode/groupNode", {
			method: "POST",
			headers: {
...
async function delGroupNode(name) {
	try {
		const response = await api.fetchApi("/selfNode/delGroupNode", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},
