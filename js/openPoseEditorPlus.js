...
class OpenPose {
  ...
  async addHand(obj,isEdit=true,group=undefined){
    ...
    let thi=this
    let x=obj['l'],y=obj['t'];
    let img = await asyncLoadImg(`/selfNode/getImage?type=hands&&name=${obj.name}`)
  ...
  getHands(){ //获取手势
    ...
    const addHandsBtn = async () => {
      try {
        const resp = await api.fetchApi(`/selfNode/getHeads`);
        if (resp.status === 200) {
          ...
          for(let i=0;i<data.length;i++){
            ...
            img.src = `/selfNode/getImage?type=hands&&name=${data[i]}`;
            img.alt = data[i];
  ...
  getImageOpse(image){ //上传图片识别骨骼姿态
    ...
    const uploadFile = async (blobFile) => {
      try {
        const resp = await fetch("/selfNode/getImagePose", {
          method: "POST",
          body: blobFile,
        });
        if (resp.status === 200) {
...
app.registerExtension({
  name: "selfNode.OpenPoseEditorPlus",
  async init(app) {
  ...
  async setup(app) {
    let openPoseNode = app.graph._nodes.filter((wi) => wi.type == "selfNode.OpenPoseEditorPlus");
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "selfNode.OpenPoseEditorPlus") {
